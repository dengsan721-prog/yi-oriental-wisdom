...
def selected_readings(fields: dict[str, str]) -> list[str]:
...
def build_payload(official: dict[int, int], properties: dict[str, dict[str, str]]) -> str:
    rows: list[str] = []
    for index in range(1, EXPECTED_COUNT + 1):
        ...
        readings = selected_readings(fields)
        if not readings:
...
def main() -> None:
    ...
    payload = build_payload(pypdf_mapping, properties)
    payload_gzip_bytes = len(gzip.compress(payload.encode("utf-8"), compresslevel=9, mtime=0))
    ...
    generated = (
        ...
        f"export const NAME_TGH_GENERATION_METADATA = {json.dumps(metadata, ensure_ascii=False, indent=2)} as const;\n\n"
