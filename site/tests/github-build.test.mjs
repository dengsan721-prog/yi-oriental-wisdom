...
test("GitHub build is the full bundled React app", async () => {
  ...
  assert.match(resultTabsRule, /grid-template-columns:minmax\(180px,\.36fr\) minmax\(0,1fr\)/);
  assert.match(resultTabsRule, /overflow-x:hidden/);
  assert.match(resultTabsRule, /scrollbar-width:none/);
  assert.match(css, /\.result-tabs-guide/);
  assert.match(css, /\.result-tab-list/);
