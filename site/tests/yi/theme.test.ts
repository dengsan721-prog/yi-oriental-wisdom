...
describe("light five-element CSS contract", () => {
  ...
  it("uses a stronger mobile typography system instead of thin serif defaults", () => {
    expect(css).toContain("main[data-element] :where(.result-shell,.life-home,.intake){font-size:16px");
    expect(css).toContain(".report-section>header h1,.life-purpose h1,.life-change-dashboard h1{font-family:var(--yi-serif);font-weight:650");
    expect(css).toContain(".result-tabs button,.result-head-actions button,.life-head button,.life-head-simple button,.life-head-simple .life-report-return,.life-daily-form label{font-family:var(--yi-sans);font-size:15px;font-weight:650}");
  });
