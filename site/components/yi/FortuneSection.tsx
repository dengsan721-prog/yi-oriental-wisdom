...
export function FortuneSection({ chart, birth }: { chart: FourPillarsResult; birth: BirthInput }) {
  ...
  if (!timeline.length) return <section className="report-section"><header><small>阶段节律</small><h1>大运方向待确认</h1><p>顺逆排需要出生性别。当前未指定，因此不生成可能误导的起运年份；补全资料后再查看。</p></header></section>;
