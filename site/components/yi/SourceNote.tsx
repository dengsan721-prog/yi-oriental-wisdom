...
export function SourceNote({ chart, items }: { chart: FourPillarsResult; items: InterpretationItem[] }) {
  ...
  const sourceIds = [...ruleIds, ...(ruleIds.includes("relation.gan-zhi.v1") ? ["classic.san-ming-tong-hui"] : [])];
  const sources = sourceIds.map(id => registry.get(id)).filter((source): source is NonNullable<typeof source> => Boolean(source));
  const theory = [...new Set(rules.filter(rule => rule.sourceType === "library-calculation" || rule.sourceType === "classical-framework").map(rule => rule.label))];
  const translation = [...new Set(rules.filter(rule => rule.sourceType === "product-method" || rule.sourceType === "product-heuristic").map(rule => rule.label))];
  ...
  return <aside className="source-note">
    ...
    <details><summary>查看完整来源与规则（{sources.length}）</summary><ul>
      {sources.map(source => <li key={source.id}>
        ...
        <span>用途：{source.id === "classic.san-ming-tong-hui" ? `作为关系术语的古典背景与脉络参考；${source.role}` : source.role}</span><br />
