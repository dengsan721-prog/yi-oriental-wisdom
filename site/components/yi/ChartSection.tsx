...
export function ChartSection({ chart, overview }: { chart: FourPillarsResult; overview: ProfessionalOverview }) {
  ...
  return <section className="report-section"><header><small>命盘结构</small><h1>{overview.dayMaster}日主</h1><p>观察置信度：{confidenceLabel[overview.confidence]}</p></header>
    ...
    <dl className="chart-facts"><div><dt>结构观察</dt><dd>{overview.pattern}</dd></div><div><dt>调候提示</dt><dd>{overview.climate}</dd></div><div><dt>十神摘要</dt><dd>{overview.tenGodSummary}</dd></div><div><dt>干支关系</dt><dd>{overview.relationSummary}</dd></div><div><dt>喜用</dt><dd>现有引擎未作单一判断</dd></div></dl>
