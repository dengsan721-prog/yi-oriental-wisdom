...
- 旧 v1 档案迁移：加载时立即执行最小化；旧档案含出生地点时，返回值和浏览器存储均清空地点。
- 姓名选填：录入控件取消必填并标注“姓名（选填）”；空姓名可提交，人生档案统一回退为“访客”。
- 称骨来源边界：明确民间通行表无统一古籍定本，仅标注内部规则表 v1、异文不混算和民俗辅助边界，不冒充权威出版溯源。
- 追加 TDD RED 首次准确命中旧档案迁移与姓名控件；覆盖测试 49/49 通过。
