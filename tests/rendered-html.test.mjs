...
test("keeps the GitHub Pages fallback honest, complete and accessible", async () => {
  ...
  assert.match(html, /href="#birth"[^>]*>\s*开始排盘/);
  assert.match(html, /<form[^>]+id="birth-form"/i);
  assert.match(html, /<select[^>]+id="birth-year"/i);
  assert.match(html, /<select[^>]+id="calendar-mode"/i);
  assert.match(html, /<section[^>]+id="calculated-result"/i);
  assert.match(html, /function\s+calculateChart\s*\(/);
  assert.match(html, /function\s+renderResult\s*\(/);
  assert.doesNotMatch(html, /yi-oriental-wisdom\.dengsan721\.chatgpt\.site/);
