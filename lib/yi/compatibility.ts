import type { ChartRelation, ElementName, FourPillarsResult, TenGodName } from "./types";
...
export type CompatibilityResult = {
  ...
  tenGodDynamics: { direction: "A→B" | "B→A"; basis: string; theme: TenGodName | "同气"; observation: string }[];
...
function directionalGod(source: FourPillarsResult, target: FourPillarsResult): TenGodName | "同气" {
  const sourceElement = source.professional.dayMaster.element;
  const targetEntry = target.professional.tenGods.find(item => item.symbol === source.professional.dayMaster.stem);
  return sourceElement === target.professional.dayMaster.element ? "同气" : targetEntry?.tenGod ?? "同气";
}
...
function crossRelations(a: FourPillarsResult, b: FourPillarsResult) {
  ...
  const pairs: [string, string, string][] = [["子", "午", "冲"], ["丑", "未", "冲"], ["寅", "申", "冲"], ["卯", "酉", "冲"], ["辰", "戌", "冲"], ["巳", "亥", "冲"], ["子", "丑", "合"], ["寅", "亥", "合"], ["卯", "戌", "合"], ["辰", "酉", "合"], ["巳", "申", "合"], ["午", "未", "合"]];
  ...
  for (const [left, right, relation] of pairs) {
  ...
  return relations.length ? relations : [{ symbols: [a.pillars.day.branch, b.pillars.day.branch], relation: "无直接六合六冲", observation: "不以单一合冲替代真实互动观察。" }];
}
