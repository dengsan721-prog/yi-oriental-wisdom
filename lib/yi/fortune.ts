...
export function buildFortuneYearReading(
  ...
  if (!primary) {
    return {
      ...
      scenario: `典型场景：${annualStemBranch}${annualGod}流年进入${periodStemBranch}${periodGod}大运，逐层核对未见关系命中时，把${periodTheme(annualGod)}作为观察问题，不制造确定事件。`,
      action: `年度动作：针对${annualStemBranch}${annualGod}与${periodStemBranch}${periodGod}的无命中结果，每月记录一次事实、选择与反馈，出现真实变化再调整行动。`,
    };
  }
  const dynamics = relationDynamics(primary.type);
  ...
  return {
    ...
    scenario: `典型场景：${annualStemBranch}${annualGod}流年与${periodStemBranch}${periodGod}大运共同触发${primary.label}（${involved}）时，${dynamics.scene}；这只是关系结构的生活化翻译。`,
    action: `年度动作：面对${annualStemBranch}${annualGod}与${periodStemBranch}${periodGod}共同形成的${primary.label}（${involved}），${dynamics.action}，不把关系结构当成结果保证。`,
  };
}
