...
const gods: Record<string, TenGodName> = { same: "比肩", resource: "正印", output: "食神", wealth: "正财", officer: "正官" };

function relationGod(day: string, other: string): TenGodName {
  ...
  return delta === 0 ? gods.same : delta === 1 ? gods.output : delta === 2 ? gods.wealth : delta === 3 ? gods.officer : gods.resource;
}
...
export function buildFortuneTimeline(chart: FourPillarsResult, input: BirthInput): FortunePeriod[] {
  ...
  return yun.getDaYun(10).filter(item => item.getIndex() > 0).map((period, periodIndex) => {
    ...
    const tenGod = relationGod(chart.pillars.day.stem, stemBranch[0]);
    const theme = periodTheme(tenGod);
    const years = period.getLiuNian(10).map(annual => {
      const annualStemBranch = annual.getGanZhi();
      const annualGod = relationGod(chart.pillars.day.stem, annualStemBranch[0]);
